...
#define gost_round(tables, k1, k2) \
t = (k1) + r; \
...
r = h[i]; \
l = h[i + 1]; \
gost_round(tables, key[0], key[1]) \
gost_round(tables, key[2], key[3]) \
gost_round(tables, key[4], key[5]) \
gost_round(tables, key[6], key[7]) \
gost_round(tables, key[0], key[1]) \
gost_round(tables, key[2], key[3]) \
gost_round(tables, key[4], key[5]) \
gost_round(tables, key[6], key[7]) \
...
gost_round(tables, key[4], key[5]) \
gost_round(tables, key[6], key[7]) \
gost_round(tables, key[7], key[6]) \
gost_round(tables, key[5], key[4]) \
gost_round(tables, key[3], key[2]) \
gost_round(tables, key[1], key[0]) \
t = r; \
r = l; \
l = t;
...
static inline void GOST(GOST_CTX *context, uint32_t data[8])
{
    int i;
    ...
    for (i = 0; i < 8; i += 2) {
        X(w, u, v);
        P(key, w);
        R((*context->tables), key, h, i, t, l, r);
        S(s, l, r);
        if (i != 6) {
            A(u, l, r);
            if (i == 2)	{
...
static inline void GOST_Transform(GOST_CTX *context, const unsigned char input[32])
{
    int i, j;
...
void GOST_Init(GOST_CTX *context)
{
    memset(context, 0, sizeof(*context));
    context->tables = &tables_test;
}

void GOSTInitCrypto(GOST_CTX *context)
{
    GOST_Init(context);
...
void GOST_Update(GOST_CTX *context, const unsigned char *input, size_t len)
{
    if ((MAX32 - context->count[0]) < (len * 8)) {
        context->count[1]++;
        context->count[0] = MAX32 - context->count[0];
        context->count[0] = ((unsigned int)len * 8) - context->count[0];
    } else {
    ...
    if (context->length + len < 32) {
        ...
        if (context->length) {
            ...
            GOST_Transform(context, context->buffer);
        }
        
        for (; i + 32 <= len; i += 32) {
            GOST_Transform(context, input + i);
        }
...
void GOST_Final(GOST_CTX *context, unsigned char digest[32])
{
    uint32_t i, j, l[8] = {0};
